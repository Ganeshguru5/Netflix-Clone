...
export default function LastetTvshows(props) {
    const [popularMovies,setPopularMovies]=useState([])
  useEffect(()=>{
    fetch("https://api.themoviedb.org/3/tv/top_rated?api_key=b02a8e9727484f1244028d52867cc576&language=en-US&page=1" )
    .then(res=>res.json())
...

            {
                popularMovies.map((movie)=>{
                    return(
                        ...
                        <Link to={`/tv/${movie.id}`} className='boxcon'>
